    ...
    class CI_Session_memcached_driver extends CI_Session_driver implements SessionHandlerInterface {
        ...
        public function __construct(&$params) {
            ...
            if (empty($this->_config['save_path'])) {
                log_message('error', 'Session: Tidak ada jalur penyimpanan Memcached yang dikonfigurasi.');
            }
        ...
        public function open($save_path, $name) {
            ...
            if (!preg_match_all('#,?([^,:]+)\:(\d{1,5})(?:\:(\d+))?#', $this->_config['save_path'], $matches, PREG_SET_ORDER)) {
                ...
                log_message('error', 'Session: Format jalur simpan Memcached tidak valid: ' . $this->_config['save_path']);
                return $this->_fail();
            }
            foreach ($matches as $match) {
                if (in_array($match[1] . ':' . $match[2], $server_list, TRUE)) {
                    log_message('debug', 'Session: Pool server memcached sudah memiliki ' . $match[1] . ':' . $match[2]);
                    continue;
                }
                if (!$this->_memcached->addServer($match[1], $match[2], isset($match[3]) ? $match[3] : 0)) {
                    log_message('error', 'Tidak dapat menambahkan ' . $match[1] . ':' . $match[2] . ' ke Memcached server pool.');
                } else {
                    $server_list[] = $match[1] . ':' . $match[2];
                }
            }
            if (empty($server_list)) {
                log_message('error', 'Session: Memcached server pool kosong.');
                return $this->_fail();
            }
        ...
        protected function _get_lock($session_id) {
            ...
            if ($attempt === 30) {
                log_message('error', 'Session: Gagal mencari kunci untuk ' . $this->_key_prefix . $session_id . ' setelah 30 kali, batal.');
                return FALSE;
            }
            $this->_lock = TRUE;
            return TRUE;
        ...
        protected function _release_lock() {
            if (isset($this->_memcached, $this->_lock_key) && $this->_lock) {
                if (!$this->_memcached->delete($this->_lock_key) && $this->_memcached->getResultCode() !== Memcached::RES_NOTFOUND) {
                    log_message('error', 'Session: Error saat mencoba membuka ' . $this->_lock_key);
                    return FALSE;
                }
                $this->_lock_key = NULL;
                $this->_lock     = FALSE;
