<?php
    ...
    abstract class CI_DB_driver {
        ...
        public function __construct($params) {
            ...
            log_message('info', 'Database Driver Class telah di inisialisasi');
        }
        public function initialize() {
            ...
            if (!$this->conn_id) {
                ...
                if (!$this->conn_id) {
                    log_message('error', 'Gagal mengkoneksikan database');
                    if ($this->db_debug) {
        ...
        public function db_set_charset($charset) {
            if (method_exists($this, '_db_set_charset') && !$this->_db_set_charset($charset)) {
                log_message('error', 'Tidak dapat mengatur charset koneksi database: ' . $charset);
                if ($this->db_debug) {
        ...
        public function query($sql, $binds = FALSE, $return_object = NULL) {
            if ($sql === '') {
                log_message('error', 'Query tidak valid: ' . $sql);
            ...
            if (FALSE === ($this->result_id = $this->simple_query($sql))) {
                ...
                $error = $this->error();
                log_message('error', 'Query error: ' . $error['message'] . ' - Query tidak valid: ' . $sql);
                if ($this->db_debug) {
                    while ($this->_trans_depth !== 0) {
                        $trans_depth = $this->_trans_depth;
                        $this->trans_complete();
                        if ($trans_depth === $this->_trans_depth) {
                            log_message('error', 'Database: Gagal saat transaksi otomatis commit/rollback!');
                            break;
                        }
                    }
                    return $this->display_error(array(
                        'Nomor Error: ' . $error['code'],
                        $error['message'],
                        $sql
                    ));
                }
        ...
        public function trans_complete() {
            ...
            if ($this->_trans_status === FALSE OR $this->_trans_failure === TRUE) {
                ...
                log_message('debug', 'Transaksi DB gagal');
                return FALSE;
            }
            ...
            foreach ($trace as $call) {
                if (isset($call['file'], $call['class'])) {
                    ...
                    if (strpos($call['file'], BASEPATH . 'database') === FALSE && strpos($call['class'], 'Loader') === FALSE) {
                        $message[] = 'Nama File: ' . str_replace(array(
                            APPPATH,
                            BASEPATH
                        ), '', $call['file']);
