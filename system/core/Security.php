<?php
    ...
    class CI_Security {
        ...
        protected $_never_allowed_str = array('document.cookie' => '[removed]', '(document).cookie' => '[removed]', 'document.write' => '[removed]', '(document).write' => '[removed]', '.parentNode' => '[removed]', '.innerHTML' => '[removed]', '-moz-binding' => '[removed]', '<!--' => '&lt;!--', '-->' => '--&gt;', '<![CDATA[' => '&lt;![CDATA[', '<comment>' => '&lt;comment&gt;', '<%' => '&lt;&#37;');
        protected $_never_allowed_regex = array('javascript\s*:', '(\(?document\)?|\(?window\)?(\.document)?)\.(location|on\w*)', 'expression\s*(\(|&\#40;)', 'vbscript\s*:', 'wscript\s*:', 'jscript\s*:', 'vbs\s*:', 'Redirect\s+30\d', "([\"'])?data\s*:[^\\1]*?base64[^\\1]*?,[^\\1]*?\\1?");
        public function __construct() {
            ...
            log_message('info', 'Security Class telah di inisialisasi');
        }
        public function csrf_verify() {
            ...
            log_message('info', 'CSRF token terverifikasi');
            return $this;
        }
        public function csrf_set_cookie() {
            ...
            log_message('info', 'CSRF cookie terkirim');
            return $this;
        }
        public function csrf_show_error() {
            show_error('Aksi tidak di izinkan.', 403);
        }
