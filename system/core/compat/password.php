<?php
	...
	if ( ! function_exists('password_hash')) {
		function password_hash($password, $algo, array $options = array()) {
			...
			if ($algo !== 1) {
				trigger_error('password_hash(): Algoritma Hash tidak diketahui: '.(int) $algo, E_USER_WARNING);
				return NULL;
			}
			if (isset($options['cost']) && ($options['cost'] < 4 OR $options['cost'] > 31)) {
				trigger_error('password_hash(): Parameter bcrypt cost tidak valid ditentukan: '.(int) $options['cost'], E_USER_WARNING);
				return NULL;
			}
			if (isset($options['salt']) && ($saltlen = ($func_overload ? mb_strlen($options['salt'], '8bit') : strlen($options['salt']))) < 22) {
				trigger_error('password_hash(): Penggunaan salt terlalu pendek: '.$saltlen.' minimal 22', E_USER_WARNING);
				return NULL;
				...
				if (function_exists('random_bytes')) {
					try {
						...
						log_message('error', 'compat/password: Error saat mencoba menggunakan random_bytes(): '.$e->getMessage());
						return FALSE;
					}
					...
					if (($fp = fopen($dev, 'rb')) === FALSE) {
						log_message('error', 'compat/password: Gagal membuka '.$dev.' untuk dibaca.');
						return FALSE;
					}
					...
					for ($read = 0; $read < 16; $read = ($func_overload) ? mb_strlen($options['salt'], '8bit') : strlen($options['salt'])) {
						if (($read = fread($fp, 16 - $read)) === FALSE) {
							log_message('error', 'compat/password: Error saat membaca '.$dev.'.');
							return FALSE;
						}
						$options['salt'] .= $read;
					}
					...
					if ($is_secure !== TRUE) {
						log_message('error', 'compat/password: openssl_random_pseudo_bytes() set $cryto_strong flag menjadi FALSE');
						return FALSE;
					}
				} else {
					log_message('error', 'compat/password: Tidak ada CSPRNG.');
					return FALSE;
				}
