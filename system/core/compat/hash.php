	...
	if ( ! function_exists('hash_equals')) {
		function hash_equals($known_string, $user_string) {
			if ( ! is_string($known_string)) {
				trigger_error('hash_equals(): mengharapkan known_string menjadi sebuah string, bukan '.strtolower(gettype($known_string)), E_USER_WARNING);
				return FALSE;
			} elseif ( ! is_string($user_string)) {
				trigger_error('hash_equals(): mengharapkan user_string menjadi sebuah string, bukan '.strtolower(gettype($user_string)), E_USER_WARNING);
				return FALSE;
	...
	if ( ! function_exists('hash_pbkdf2')) {
		function hash_pbkdf2($algo, $password, $salt, $iterations, $length = 0, $raw_output = FALSE) {
			if ( ! in_array(strtolower($algo), hash_algos(), TRUE)) {
				trigger_error('hash_pbkdf2(): Algoritma hash tidak diketahui: '.$algo, E_USER_WARNING);
				return FALSE;
			}
			if (($type = gettype($iterations)) !== 'integer') {
				...
				if (is_string($iterations) && is_numeric($iterations)) {
					...
					trigger_error('hash_pbkdf2() mengharapkan parameter 4 menjadi long, bukan '.$type, E_USER_WARNING);
					return NULL;
				}
			}
			if ($iterations < 1) {
				trigger_error('hash_pbkdf2(): Iterasi harus integer positif: '.$iterations, E_USER_WARNING);
				return FALSE;
			}
			if (($type = gettype($length)) !== 'integer') {
				...
				if (is_string($length) && is_numeric($length)) {
					...
					trigger_error('hash_pbkdf2() mengharapkan parameter 5 menjadi long, bukan '.$type, E_USER_WARNING);
					return NULL;
				}
			}
			if ($length < 0) {
				trigger_error('hash_pbkdf2(): Panjangnya harus lebih atau sama dengan 0: '.$length, E_USER_WARNING);
				return FALSE;
			}
