		...
		function array_column(array $array, $column_key, $index_key = NULL) {
			if ( ! in_array($type = gettype($column_key), array('integer', 'string', 'NULL'), TRUE)) {
				if ($type === 'double') {
					...
					trigger_error('array_column(): Kunci kolom harus berupa string atau integer', E_USER_WARNING);
					return FALSE;
				}
			}
			if ( ! in_array($type = gettype($index_key), array('integer', 'string', 'NULL'), TRUE)) {
				if ($type === 'double') {
					...
					trigger_error('array_column(): Kunci indeks harus berupa string atau integer', E_USER_WARNING);
					return FALSE;
				}
			}
			$result = array();
		...
		function hex2bin($data) {
			if (in_array($type = gettype($data), array('array', 'double', 'object', 'resource'), TRUE)) {
				if ($type === 'object' && method_exists($data, '__toString')) {
					...
					trigger_error('hex2bin() mengharapkan parameter 1 menjadi string, bukan '.$type, E_USER_WARNING);
					return NULL;
				}
			}
			if (strlen($data) % 2 !== 0) {
				trigger_error('String input heksadesimal harus memiliki panjang yang sama', E_USER_WARNING);
				return FALSE;
			} elseif ( ! preg_match('/^[0-9a-f]*$/i', $data)) {
				trigger_error('String input harus berupa string heksadesimal', E_USER_WARNING);
				return FALSE;
			}
			return pack('H*', $data);
		}
